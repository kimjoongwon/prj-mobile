...
- `StatCard` - 통계 표시 카드 (숫자 + 아이콘 + 레이블)
- `EmptyState` - 빈 상태 플레이스홀더
- `SearchBar` - 검색바 (Input + Icon + Button 조합)
- `UserAvatar` - 아바타 + 이름 조합
...
// ✅ widgets - 조합된 UI 패턴
<StatCard value={1234} label="Users" icon="👥" trend="+12%" />
<SearchBar placeholder="Search..." onSearch={handleSearch} />
```

#### 3. **features/** - 도메인 특화 기능 컴포넌트
...
2. **조합 UI 컴포넌트**: `widgets/` 탐색
   - 통계 카드? → `widgets/StatCard/`
   - 검색바? → `widgets/SearchBar/`

3. **도메인 특화 기능**: `features/` 탐색
